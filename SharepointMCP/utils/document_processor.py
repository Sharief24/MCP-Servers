...
class DocumentProcessor:
    ...
    # Maximum rows returned per sheet so the LLM can reason over the full content
    ...
    def _process_excel(content: bytes) -> Dict[str, Any]:
        ...
        max_rows = DocumentProcessor._EXCEL_MAX_ROWS

        for sheet_name, df in df_dict.items():
            # Drop completely empty rows so we don't waste tokens on blank padding.
            df = df.dropna(how="all")

            # Convert all values to strings so JSON serialisation never fails on
            # NaT / Timestamp / numpy numeric types.
            preview_df = df.head(max_rows).astype(object).where(df.head(max_rows).notna(), other=None)
    ...
    def _process_word(content: bytes) -> Dict[str, Any]:
        ...
        return {
            ...
            "content": paragraphs[:100],  # First 100 paragraphs
    ...
    def _process_pdf(content: bytes) -> Dict[str, Any]:
        ...
        pages = []
        for i in range(min(30, len(pdf.pages))):  # First 30 pages
