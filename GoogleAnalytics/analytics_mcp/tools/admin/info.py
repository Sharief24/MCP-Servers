...
from analytics_mcp.tools.client import (
    ...
    create_admin_api_client_with_token,
    create_admin_alpha_api_client_with_token,
)
from google.analytics import admin_v1beta, admin_v1alpha


async def get_account_summaries(access_token: str) -> List[Dict[str, Any]]:
    """Retrieves information about the user's Google Analytics accounts and properties.

    Args:
        access_token: Google OAuth2 Bearer access token for authentication.
    """

    def _sync_call():
        summary_pager = create_admin_api_client_with_token(access_token).list_account_summaries()
        return [proto_to_dict(summary_page) for summary_page in summary_pager]

    return await asyncio.to_thread(_sync_call)


async def list_google_ads_links(access_token: str, property_id: int | str) -> List[Dict[str, Any]]:
    ...
    def _sync_call():
        links_pager = create_admin_api_client_with_token(access_token).list_google_ads_links(
            request=request
        )
...
async def get_property_details(access_token: str, property_id: int | str) -> Dict[str, Any]:
    ...
    def _sync_call():
        client = create_admin_api_client_with_token(access_token)
...
async def list_property_annotations(
    ...
    def _sync_call():
        annotations_pager = (
            create_admin_alpha_api_client_with_token(access_token).list_reporting_data_annotations(
                request=request
            )
        )
        return [
