...
from analytics_mcp.tools.client import create_data_api_client_with_token
...
def _run_realtime_report_description() -> str:
    ...
    return f"""
          ...
          **Format:** A list of dimension name strings.
          Example: `["country", "deviceCategory", "unifiedScreenName"]`
          Do NOT pass dicts like `[{{"name": "country"}}]` — pass plain strings only.
          ...
          **Format:** A list of metric name strings.
          Example: `["activeUsers"]`
          Do NOT pass dicts like `[{{"name": "activeUsers"}}]` — pass plain strings only.
...
async def run_realtime_report(
    ...
    return_property_quota: Optional[bool] = False,
) -> Dict[str, Any]:
    ...
    def _sync_call():
        return create_data_api_client_with_token(access_token).run_realtime_report(request)
