...
def _run_report_description() -> str:
    ...
    return f"""
          ...
          **Format:** A list of dimension name strings.
          ...
          Do NOT pass dicts like `[{{"name": "date"}}]` — pass plain strings only.
          ...
          **Format:** A list of metric name strings.
          Example: `["activeUsers", "eventCount", "engagementRate"]`
...
async def run_report(
    ...
    def _sync_call():
        return create_data_api_client_with_token(access_token).run_report(request)
