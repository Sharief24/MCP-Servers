...
for tool in mcp_tools:
    ...
    if tool.name == "run_report":
        ...
        tool.inputSchema["required"] = ["access_token", "property_id", "dimensions", "metrics"]
    elif tool.name == "run_funnel_report":
        tool.inputSchema["required"] = ["access_token", "property_id", "funnel_steps"]

    # Ensure access_token is required for every tool that exposes it
    props = tool.inputSchema.get("properties", {})
    if "access_token" in props:
        required = tool.inputSchema.setdefault("required", [])
        if "access_token" not in required:
            required.insert(0, "access_token")
